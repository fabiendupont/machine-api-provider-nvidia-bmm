# Machine API Provider for NVIDIA Bare Metal Manager

OpenShift Machine API actuator for provisioning bare-metal machines on NVIDIA Bare Metal Manager (BMM) platform.

## Overview

This provider implements the OpenShift Machine API actuator interface for NVIDIA BMM, enabling:
...
The provider translates OpenShift Machine API requests into NVIDIA BMM REST API calls (via the carbide-rest client library), managing the full lifecycle of bare-metal instances.
...
+----------+------------------------+-----------------+
           ...
           v                        v
+-----------------------------------------------------+
|   Machine API Provider for NVIDIA BMM (this repo)   |
|  +----------------------------------------------+   |
|  |  Machine Reconciler                          |   |
|  |  +----------------------------------------+  |   |
|  |  |  Actuator                              |  |   |
|  |  |  - Create/Update/Delete/Exists         |  |   |
|  |  |  - NvidiaBMMMachineProviderSpec parser |  |   |
|  |  +----------+-----------------------------+  |   |
|  +-------------+--------------------------------+   |
+----------------+------------------------------------+
                 |
...
2. NVIDIA BMM API credentials (endpoint, orgName, token)
3. Access to NVIDIA BMM platform with configured Sites, VPCs, and Subnets

### Build from Source

```bash
# Clone repository
git clone https://github.com/NVIDIA/machine-api-provider-nvidia-bmm.git
cd machine-api-provider-nvidia-bmm

# Build binary
make build

# Build Docker image
make docker-build IMG=your-registry/machine-api-provider-nvidia-bmm:tag

# Push to registry
make docker-push IMG=your-registry/machine-api-provider-nvidia-bmm:tag
```
...
kubectl create secret generic nvidia-bmm-credentials \
  ...
  --from-literal=endpoint="https://api.nvidia-bmm.example.com" \
...
```bash
# Update image in config/manager/deployment.yaml
...
kind: MachineSet
metadata:
  name: worker-nvidia-bmm-us-west
  namespace: openshift-machine-api
spec:
  replicas: 3
  selector:
    matchLabels:
      machine.openshift.io/cluster-api-machineset: worker-nvidia-bmm-us-west
  template:
    metadata:
      labels:
        machine.openshift.io/cluster-api-machineset: worker-nvidia-bmm-us-west
...
### NvidiaBMMMachineProviderSpec

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `siteId` | string | Yes | NVIDIA BMM Site UUID |
| `tenantId` | string | Yes | NVIDIA BMM Tenant ID |
...
### NvidiaBMMMachineProviderStatus

| Field | Type | Description |
|-------|------|-------------|
| `instanceId` | string | NVIDIA BMM instance UUID |
...
```
machine-api-provider-nvidia-bmm/
├── cmd/manager/          # Controller manager entry point
├── pkg/
│   ├── apis/            # NvidiaBMMMachineProviderSpec types
...
kubectl logs -n openshift-machine-api \
  -l app=machine-api-provider-nvidia-bmm \
  --tail=100 -f
```
...
- Network connectivity to NVIDIA BMM API
...
kubectl auth can-i get machines.machine.openshift.io \
  --as=system:serviceaccount:openshift-machine-api:machine-api-provider-nvidia-bmm
```
